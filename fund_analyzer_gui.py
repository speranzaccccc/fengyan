...
class FundAnalyzerApp:
    ...
    def setup_ui(self):
        ...
        product_frame = tk.Frame(right_frame, bg=self.colors["panel"], padx=18, pady=16)
        ...
        metric_frame = tk.Frame(right_frame, bg=self.colors["bg"])
        ...
        self.metrics_frame = tk.Frame(right_frame, bg=self.colors["panel"], padx=14, pady=12)
    ...
    def upload_file(self):
        ...
        if filepath:
            try:
                ...
                self.fund_name, self.data = load_fund_data(filepath)
                self.file_label.config(
                    text=f"已选择：{Path(filepath).name}\n{len(self.data)} 条记录\n{self.data[0][0]} 至 {self.data[-1][0]}"
                )
                self.update_display()
    ...
    def update_display(self):
        ...
        self.results = build_report(self.fund_name, self.data, self.rf_annual, self.use_prev_year_end)
    ...
    def export_results(self):
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            initialfile=f"{self.fund_name}_分析结果.xlsx"
        )
        if filepath:
            try:
